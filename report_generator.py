...
def generate_pdf(analysis: ComplianceAnalysis) -> bytes:
    ...
    Args:
        analysis: ComplianceAnalysis from the Claude API.
...
def _add_findings_table(story, styles, analysis: ComplianceAnalysis):
    ...
    headers = ["Requirement", "Status", "Risk", "Finding", "Recommendation"]
    col_widths = [3.5 * cm, 2.2 * cm, 1.5 * cm, 5.5 * cm, 4.5 * cm]

    rows = [headers]
    for finding in analysis.findings:
        rows.append([
            Paragraph(f"<b>{finding.requirement_id}</b><br/>{finding.requirement_name}", styles["table_cell"]),
    ...
    # Colour-code status and risk cells
    for row_idx, finding in enumerate(analysis.findings, start=1):
        status_col = STATUS_COLOURS.get(finding.status, MID_GREY)
        risk_col = RISK_COLOURS.get(finding.risk_level, MID_GREY)
        table_style.append(("BACKGROUND", (1, row_idx), (1, row_idx), _lighten(status_col)))
        table_style.append(("BACKGROUND", (2, row_idx), (2, row_idx), _lighten(risk_col)))
...
def _add_footer_note(story, styles):
    ...
    story.append(Spacer(1, 0.2 * cm))
    story.append(Paragraph(
        "This report was generated by AI Compliance Checker using Claude AI. "
        "It is intended as a starting point for compliance review and should be "
        "validated by a qualified compliance professional.",
        styles["footer"],
    ))
